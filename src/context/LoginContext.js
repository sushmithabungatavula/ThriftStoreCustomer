...
export const LoginProvider = ({ children }) => {
  ...
  const fetchCartItems = async () => {
    if (cart_id && isLoggedIn) {
      try {
        const response = await axios.get(`http://localhost:3000/api/cart/${cart_id}/items`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
    ...
    const fetchWishlistItems = async () => {
      if (wishlist_id && isLoggedIn) {
        try {
          const response = await axios.get(`http://localhost:3000/api/wishlist/${wishlist_id}/items`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
...
const handleWishlistToggle = async (productId) => {
  try {
    ...
    if (isInWishlist) {
      ...
      await axios.delete(`http://localhost:3000/api/wishlist/${wishlist_id}/remove`, {
      ...
      // Add to wishlist
      await axios.post(`http://localhost:3000/api/wishlist/${wishlist_id}/add`, {
        item_id: productId,
      });
      setWishlist(prev => {
 ...
 const handleAddToCart = async (product) => {
  try {
    ...
    if (existingItem) {
      ...
      await axios.post(`http://localhost:3000/api/cart/${cart_id}/addexistingitem`, {
      ...
      // Add as a new item
      await axios.post(`http://localhost:3000/api/cart/${cart_id}/add`, {
        item_id: product.item_id,
        quantity: 1,
      });
      setCartItems((prevCart) => {
...
const handleReduceQuantity = async (itemId) => {
  try {
    ...
    if (item && item.quantity === 1) {
      ...
      await axios.delete(`http://localhost:3000/api/cart/${cart_id}/remove`, {
        data: { item_id: itemId },
      });
      setCartItems((prevCart) =>
      ...
      await axios.post(`http://localhost:3000/api/cart/${cart_id}/removeexistingitem`, {
        item_id: itemId,
        quantity: -1,
      });
      setCartItems((prevCart) =>
